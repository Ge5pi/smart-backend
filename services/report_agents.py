...
class Orchestrator(BaseAgent):
    ...
    def create_initial_plan(self):
        ...
        MUST-INCLUDE Question Types: Time-Series/Trend Analysis, Cross-Table Correlation (requiring a JOIN), Distribution/Anomaly Detection.
        Return ONLY the JSON array of strings. Example: {{"plan": ["Question 1?", "Question 2?"]}}
        **Database Schema:**
        ```
        {schema}
        ```
...
class Critic(BaseAgent):
    def evaluate(self, execution_result: dict):
        ...
        prompt = f"""
        You are a Senior Data Analyst. Analyze the query result.
        **Original Question:** "{question}"
        **Data Result (first 10 rows):**
        ```
        {df_markdown_escaped}
        ```
        **Your Tasks (JSON format):**
        1. `summary` (string): Write a concise, insightful summary.
        2. `new_hypotheses` (array of strings): Generate 1-3 new follow-up questions.
